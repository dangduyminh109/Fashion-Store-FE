...
export const RecommendProduct = ({ cartItems }: { cartItems: string }) => {
  const navigate = useNavigate();
  const { data, loading, error } = useFetch<ProductFeatured[]>({
    endpoint: `/recommend?items=${cartItems}`,
    method: "get",
  });
  useLayoutEffect(() => {
    if (!data || loading || error) return;
    const ctx = gsap.context(() => {
      gsap.from("#RecommendProduct .card-item-wrapper", {
        scrollTrigger: {
          trigger: "#RecommendProduct",
          start: "top 50%",
