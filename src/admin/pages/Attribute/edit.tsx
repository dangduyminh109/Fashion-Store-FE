...
import type AttributeValue from "~/admin/types/attributeValue";
import getLastError from "~/utils/onErrorValidate";
import { editSchema as schema } from "~/admin/schemas/attributeSchema";
...
function Edit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const attribute = await axiosClient.get(`/attribute/` + id);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (res.data.code == 1000) {
        navigate("/admin/product/attributes", { state: { message: res.data.message } });
        return;
      }
