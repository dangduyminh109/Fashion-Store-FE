...
import schema from "~/admin/schemas/postSchema";
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (res.data.code == 1000) {
        navigate("/admin/posts", { state: { message: res.data.message } });
        return;
      }
    ...
    <Fragment>
      ...
      {!loading && !error && (
        <form onSubmit={handleSubmit(onSubmit, onError)}>
          ...
          >
            <Button variant="outlined" size="large" onClick={() => navigate("/admin/posts")}>
              Quay Lại
            </Button>
