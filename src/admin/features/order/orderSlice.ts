...
import { fetchOrder, deleteOrRestore, updateStatus } from "./orderApi";
    ...
    builder
      ...
      .addCase(updateStatus.fulfilled, (state, action) => {
        if (state.code === 1000 && action.meta.arg) {
          ...
          const item = state.data.find((c: Order) => c.id.toString() === updatedId);
          if (item) {
      ...
      .addCase(deleteOrRestore.fulfilled, (state, action) => {
        if (state.code === 1000 && action.meta.arg) {
          const arg = action.meta.arg.url.split("/");
          const updatedId = arg[2];
          state.data = state.data.filter((x: Order) => x.id.toString() != updatedId);
        }
      });
  },
});
