...
const createSchema = yup.object({
  ...
  attributeDisplayType: yup.string().default("TEXT").required("Loại hiển thị không được để trống!"),
  listAttributeValue: yup
    .array()
    .of(
      yup.object({
        value: yup.string().when("$$parent.attributeDisplayType", {
          is: (val: string) => val === "TEXT" || val === "SELECT",
          then: (schema) => schema.required("Giá trị là bắt buộc khi kiểu hiển thị TEXT"),
          otherwise: (schema) => schema.optional(),
        }),
        color: yup.string().when("$$parent.attributeDisplayType", {
          is: "COLOR",
          then: (schema) => schema.required("Color là bắt buộc khi kiểu hiển thị COLOR"),
          otherwise: (schema) => schema.optional(),
        }),
        image: yup.mixed<File>().when("$$parent.attributeDisplayType", {
          is: "IMAGE",
